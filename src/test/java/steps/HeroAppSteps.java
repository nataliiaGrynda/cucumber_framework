...
public class HeroAppSteps {
  ...
  public void userEntersUsernameAsAndPasswordAs(String usernameInputs, String passwordInputs) {
  Assert.assertTrue(heroAppPage.usernameInput.isDisplayed());
  Assert.assertTrue(heroAppPage.passwordInput.isDisplayed());
  heroAppPage.usernameInput.sendKeys(usernameInputs);
  heroAppPage.passwordInput.sendKeys(passwordInputs);
  }

  @Then("user should see a message starts with {string}")
  public void userShouldSeeAMessageStartsWith(String errorMessage) {
Assert.assertTrue(heroAppPage.usernameIsInvalidMessage.getText().startsWith(errorMessage));
  }
}